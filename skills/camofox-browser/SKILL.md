...
camofox links                        # Get all links on the page
```
...
### Session Management
```bash
camofox --session work open <url>    # Isolated session (separate cookies/storage)
...
### Server Control
```bash
camofox start                        # Start server (usually auto-starts)
...
| `HTTPS_PROXY` | — | Proxy server (e.g. `socks5://127.0.0.1:1080`) |
...
If CLI doesn't work (e.g. Node version mismatch), use direct REST API calls to `http://127.0.0.1:9377`. See [references/api-reference.md](references/api-reference.md) for full endpoint documentation.

```bash
# Create tab
TAB=$(curl -s -X POST http://127.0.0.1:9377/tabs \
  -H "Content-Type: application/json" \
  -d '{"userId":"agent","sessionKey":"default","url":"<url>"}')
TAB_ID=$(echo "$TAB" | python3 -c "import sys,json; print(json.load(sys.stdin).get('tabId',''))")

# Get snapshot
curl -s "http://127.0.0.1:9377/tabs/$TAB_ID/snapshot?userId=agent" | sed 's/.*"snapshot":"//' | sed 's/","refsCount.*//' | sed 's/\\n/\n/g'

# Interact (note: send refs as 'e1' not '@e1')
curl -s -X POST "http://127.0.0.1:9377/tabs/$TAB_ID/click" \
  -H "Content-Type: application/json" \
  -d '{"userId":"agent","ref":"e1"}'

curl -s -X POST "http://127.0.0.1:9377/tabs/$TAB_ID/type" \
  -H "Content-Type: application/json" \
  -d '{"userId":"agent","ref":"e1","text":"query"}'
```
