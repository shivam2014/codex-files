...
| 6 | SVG `viewBox` height is tall enough for all content | Bottom annotations clipped |
...
const svgTexts = html.match(/<text[^>]*>.*?<\/text>/gs) || [];
for (const t of svgTexts) {
  ...
  if (!inside) {
    fail('WARN', 'SVG <text> appears empty');
  }
}
...
for (const ly of labelY) {
  for (const ay of arrowY) {
    if (Math.abs(ly - ay) < 6) {
      fail('WARN', `Annotation at y=${ly} overlaps arrow at y=${ay} (distance < 6px)`);
    }
  }
}
...
if (html.includes('prefers-color-scheme: dark')) {
  if (html.includes('class="dark-mode"') && !html.includes('html[data-dm=') && !html.includes('html.dark-mode')) {
    fail('WARN', 'Uses .dark-mode class without html[data-dm] — may be overridden by @media :root');
  }
}
...
if (svgOpens !== svgCloses) fail('ERR', `SVG: ${svgOpens} opens, ${svgCloses} closes`);

const detOpens = (html.match(/<details[\s>]/g) || []).length;
const detCloses = (html.match(/<\/details>/g) || []).length;
if (detOpens !== detCloses) fail('ERR', `<details>: ${detOpens} opens, ${detCloses} closes`);

// ── 5. JS onclick references exist ──
const onclickAttrs = [...html.matchAll(/onclick="(\w+)\(/g)].map(m => m[1]);
for (const fn of [...new Set(onclickAttrs)]) {
...
if (vbMatch) {
  ...
  const maxY = Math.max(0, ...[...html.matchAll(/y="(\d+)"/g)].map(m => +m[1]));
  if (maxY > vbHeight - 20) {
    fail('WARN', `SVG viewBox height=${vbHeight} but content reaches y=${maxY} (may clip)`);
  }
}
...
- `onclick` functions that are never defined
